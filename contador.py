...
video = cv2.VideoCapture('entrada.dav')
contador = 0
liberado = False
...
while True:
    ...
    x,y,w,h = 650,460,50,290
    imgTh = cv2.adaptiveThreshold(imgGray, 255, cv2.ADAPTIVE_THRESH_GAUSSIAN_C, cv2.THRESH_BINARY_INV, 11, 12)
    karnel = np.ones((8,8), np.uint8)
    imgDil = cv2.dilate(imgTh, karnel, iterations=2)

#Pegando as marcações pelo paint(colocando linha marcadora)

    recorte = imgDil[y:y+h, x:x+w]
    brancos = cv2.countNonZero(recorte)

    if brancos > 9000 and liberado == True:
        contador +=1
